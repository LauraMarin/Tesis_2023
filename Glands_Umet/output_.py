...
def unet_mask(indice_slide):
    suffix = str(indice_slide).zfill(3)
    print(suffix)
    basepath = 'C:/Users/Laura/AppData/Local/Programs/Python/Python36/Phenotype/Unet_nuclei/wsi/tiles_png'#Images'
# ----- parse command line arguments
    ...
    parser.add_argument('-m', '--model', help="model", default="epi_unet_best_model.pth", type=str)
...
#resize = args.resize
    OUTPUT_DIR_ = 'C:/Users/Laura/AppData/Local/Programs/Python/Python36/Phenotype/Output/output_glands'
...
# ------ work on files
    for num, fname in (enumerate(os.listdir(basepath +'/'+ suffix))):
      ...
      #  io = cv2.erode(io,kernel,iterations =2)
      #  io = cv2.dilate(io,kernel,iterations =1)
     #   io = zoom(io, 2)
        
        io = cv2.cvtColor(io,cv2.COLOR_BGR2RGB)
        io = reinhard (io,mean, std)
     #   io = cv2.erode(io, kernel, iterations=1)
