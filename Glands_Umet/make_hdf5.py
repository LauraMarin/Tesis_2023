...
test_set_size=20 # what percentage of the dataset should be used as a held out validation/testing set
...
files=glob.glob('imgs/masks/*.png') # create a list of the files, in this case we're only interested in files which have masks so we can use supervised learning
print(len(files))
...
for phase in phases.keys(): #now for each of the phases, we'll loop through the files
    ...
    for filei in phases[phase]: #now for each of the files
        ...
        for imgtype in imgtypes:
            ...
            if(imgtype=="img"):
                print('a')
                j="/imgs/"+os.path.basename(fname).replace("_mask.png",".tif")
                    
                k=cv2.imread(j)
                    #if we're ./ looking at an img, it must be 3 channel, but cv2 won't load it in the correct channel order, so we need to fix that
                io=cv2.cvtColor(cv2.imread("imgs/"+os.path.basename(fname).replace("_mask.png",".tif")),cv2.COLOR_BGR2RGB)
                interp_method=PIL.Image.BICUBIC
                print(j)
                
            else:
                print('hey')#if its a mask image, then we only need a single channel (since grayscale 3D images are equal in all channels)
